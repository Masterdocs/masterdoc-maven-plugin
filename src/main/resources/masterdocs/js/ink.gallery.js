...
Ink.createModule('Ink.UI.Gallery', '1', ['Ink.UI.Aux_1', 'Ink.Dom.Event_1', 'Ink.Dom.Css_1', 'Ink.Dom.Element_1', 'Ink.Dom.Selector_1', 'Ink.Util.Array_1', 'Ink.Util.Swipe_1'], function (Aux, Event, Css, Element, Selector, InkArray, Swipe) {
    ...
    Gallery.prototype = {
        ...
        _extractModelFromDOM: function () {
            ...
            InkArray.each(liEls, function (liEl) {
                try {
                    ...
                    if (dims && !isNaN(dims[0]) && !isNaN(dims[1])) {
                        d.dims = dims;
                    }
                ...
                if (that._options.fixImageSizes) {
                    var dimsCb = function (dims) {
                        ...
                        if (w !== W || h !== H) {
                            ...
                            if (dh > 0) {
                                s.paddingBottom = dh + 'px';
                            }
                        }
                    };

                    if (d.dims) {
                        dimsCb.call({img: imgEl, data: d}, d.dims);
                    }
                    else {
                        getDimsAsync({img: imgEl, data: d}, dimsCb);
                    }
                }
            });
            ...
            if (Swipe._supported) {
                new Swipe(el, {
                    callback: Ink.bind(function (sw, o) {
                        ...
                        if ((!th && !sl) || (th && !this._swipeThumbsDir)) {
                            return;
                        }
                        if ((sl && o.axis !== this._swipeDir) || (th && o.axis !== this._swipeThumbsDir)) {
                            return;
                        }
                        if (o.dr[0] < 0) {
                            if (th) {
        ...
        _onNavClick: function (ev) {
            ...
            var delta;
            if (Css.hasClassName(tgtEl, 'previous')) {
                delta = -1;
            }
            else if (Css.hasClassName(tgtEl, 'next')) {
                delta = 1;
            }
            else {
                return;
        ...
        _validateValue: function (i, isRelative, isThumb) {
            ...
            if (this._options.circular) {
                ...
                else if (i >= this._model.length) {
                    i = 0;
                }
            }
            else {
                if (i < 0 || i >= this._model.length || i === val) {
                    return false;
                }
            }
